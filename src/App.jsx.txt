...
const GAMES = {
  ...
  coin_hidden: {
    ...
    name: "裏・1円玉トス",
    kanji: "裏表裏審査",
    desc: "1円玉を実際に宙へ放り投げる。物理演算で舞う様を見届けながら、表が続く限り記録を伸ばせ。",
    specialTitle: "宙を制する者",
    lowerIsBetter: false,
    hidden: true,
  },
  ...
  rhythm4: {
    ...
    name: "拍子審査",
    kanji: "拍子審査",
    desc: "一定のテンポで4回鳴る音に合わせて、4回ともタップせよ。ズレは積算される。",
    specialTitle: "拍動の支配者",
    lowerIsBetter: true,
  },
  dualgauge: {
    ...
    name: "二重ゲージ審査",
    kanji: "二重審査",
    desc: "速さの違う2本のゲージを、1回の操作で同時に目標内へ止めよ。",
    specialTitle: "二重制御の達人",
    lowerIsBetter: true,
  },
  quadtap: {
    id: "quadtap",
    ...
    name: "四点同時審査",
    kanji: "四点審査",
    desc: "画面上の4点を、4本指で寸分違わず同時にタップせよ。(スマホ専用)",
    specialTitle: "四方の同期者",
    lowerIsBetter: true,
  },
  simon: {
    ...
    name: "記憶審査",
    kanji: "記憶審査",
    desc: "光る順番を記憶し、同じ順にタップせよ。正解するたびに1つずつ長くなる。",
    specialTitle: "記憶の番人",
    lowerIsBetter: false,
  },
  chain3: {
    ...
    name: "三連審査",
    kanji: "三連審査",
    desc: "1回の連続動作の中で、3秒→7秒→12秒を順番に正確に止めよ。",
    specialTitle: "三位一体の審査官",
    lowerIsBetter: true,
  },
  dial: {
    id: "dial",
    ...
    name: "回転審査",
    kanji: "回転審査",
    desc: "ダイヤルを指で回し、表示された目標角度でぴったり止めよ。",
    specialTitle: "回転の支配者",
    lowerIsBetter: true,
  },
  flickangle: {
    ...
    name: "角度審査",
    kanji: "角度審査",
    desc: "指定された角度ちょうどの方向へ、画面をフリックせよ。",
    specialTitle: "軌道の支配者",
    lowerIsBetter: true,
  },
  reaction: {
    ...
    name: "反応審査",
    kanji: "反応審査",
    desc: "長い待機の後、一瞬だけ緑に光った時だけタップせよ。フライングは即失格。",
    specialTitle: "刹那の反射神経",
    lowerIsBetter: true,
  },
  nback: {
    id: "nback",
    ...
    name: "暗記審査",
    kanji: "暗記審査",
    desc: "次々表示される数字が、1つ前と同じかどうかを8回連続で正しく判定せよ。",
    specialTitle: "一手先の記憶者",
    lowerIsBetter: true,
  },
  tripletap: {
    id: "tripletap",
    ...
    name: "連打審査・弐",
    kanji: "連打審査弐",
    desc: "0.2秒→0.3秒→0.4秒と、間隔を変えながら3回連続でダブルタップせよ。",
    specialTitle: "多重律動の支配者",
    lowerIsBetter: true,
  },
};
const GAME_ORDER = [
...
// ============ 経験値・レベル・テーマ ============
function xpForResult(result, isNewBest) {
  return result.stars * 10 + (isNewBest ? 15 : 0);
}
...
async function fetchGlobalRanking() {
  const q = query(collection(db, "players"), orderBy("totalXp", "desc"), limit(30));
  const snap = await getDocs(q);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}

async function submitQuestionIdea(text) {
  const trimmed = text.trim().slice(0, 100);
  if (!trimmed) return { ok: false, message: "文章を入力してください" };
  try {
    await addDoc(collection(db, "question_submissions"), {
      text: trimmed,
      createdAt: serverTimestamp(),
    });
    return { ok: true, message: "投稿しました。審査官が確認します。" };
  } catch (e) {
    return { ok: false, message: "送信に失敗しました。通信状況を確認してください" };
  }
}
...
async function createRoom(gameId, hostId, hostNickname) {
...
async function joinRoom(code, guestId, guestNickname) {
  const ref = doc(db, "rooms", code);
  const snap = await getDoc(ref);
  if (!snap.exists()) return { ok: false, message: "その部屋番号は見つかりませんでした" };
  const data = snap.data();
  if (data.guestId) return { ok: false, message: "その部屋はすでに満室です" };
  if (data.hostId === guestId) return { ok: false, message: "自分の部屋には参加できません" };
  await updateDoc(ref, { guestId, guestNickname: guestNickname || "", status: "ready" });
  return { ok: true };
}
...
async function setRoomReady(code, role) {
  const field = role === "host" ? "hostReady" : "guestReady";
  await updateDoc(doc(db, "rooms", code), { [field]: true });
}
async function startRoomPlaying(code) {
  await updateDoc(doc(db, "rooms", code), { status: "playing" });
}
async function submitRoomResult(code, role, result) {
  const field = role === "host" ? "hostResult" : "guestResult";
  await updateDoc(doc(db, "rooms", code), {
    [field]: { stars: result.stars, statValue: result.statValue, rawValue: result.rawValue },
  });
}
async function finishRoom(code) {
  await updateDoc(doc(db, "rooms", code), { status: "done" });
}
...
function CoinTossHiddenGame({ onFinish }) {
  const [streak, setStreak] = useState(0);
  const [tossing, setTossing] = useState(false);
  const [ended, setEnded] = useState(false);
  const [lastFace, setLastFace] = useState(null);
  const [transform, setTransform] = useState("translateY(0px) rotateX(0deg)");
  ...
  const toss = () => {
    if (tossing || ended) return;
    playTap();
    setTossing(true);
    const isTails = Math.random() < 0.5;
    const spins = 3 + Math.floor(Math.random() * 3); // 3〜5回転
    const finalAngle = spins * 360 + (isTails ? 180 : 0);
    const start = performance.now();

    const frame = (now) => {
      const t = Math.min(1, (now - start) / DURATION);
      const arc = Math.sin(Math.PI * t) * HEIGHT; // 放物線
      const rot = finalAngle * easeOutCubic(t);
      setTransform(`translateY(${-arc}px) rotateX(${rot}deg)`);
      setShadowScale(1 - 0.5 * Math.sin(Math.PI * t));
      if (t < 1) {
        rafRef.current = requestAnimationFrame(frame);
      } else {
        setTossing(false);
        const face = isTails ? "裏" : "表";
        setLastFace(face);
        vibrate([20]);
        if (face === "表") {
          setStreak((s) => s + 1);
  ...
  return (
    <div style={{ textAlign: "center", marginTop: 10 }}>
      ...
      {lastFace && !tossing && (
        <div style={{ fontSize: 12, color: lastFace === "表" ? "#c9a227" : "#a83232", marginTop: 4 }}>
          結果:{lastFace}
        </div>
      )}
      {!ended ? (
        <button onClick={toss} disabled={tossing} style={btnStyle("#b8342a")}>
          {tossing ? "……" : "放り投げる"}
        </button>
      ) : (
        <div style={{ marginTop: 14, fontSize: 13, color: "#a83232" }}>
...
// ============ ミニゲーム: 拍子審査(4連続リズムタップ) ============
function RhythmGame({ onFinish }) {
  const BPM = 88;
  const INTERVAL = 60000 / BPM;
  const [beatIndex, setBeatIndex] = useState(-1); // -1:待機中
  ...
  const start = () => {
    ...
    for (let i = 0; i < 4; i++) beatTimesRef.current.push(t0 + i * INTERVAL);
    let idx = 0;
    const scheduleNext = () => {
      const now = performance.now();
      const wait = beatTimesRef.current[idx] - now;
      timerRef.current = setTimeout(() => {
        setFlash(true);
        setTimeout(() => setFlash(false), 120);
        setBeatIndex(idx);
        idx++;
        if (idx < 4) scheduleNext();
        else {
          setTimeout(() => finalize(), INTERVAL);
        }
      }, Math.max(0, wait));
    };
    scheduleNext();
  ...
  const finalize = () => {
    setDone(true);
    playTap();
    const errs = beatTimesRef.current.map((bt, i) => {
      const tap = tapTimesRef.current[i];
      return tap ? Math.abs(tap - bt) : 400; // タップ無しは大きなペナルティ
    });
    const avg = errs.reduce((a, b) => a + b, 0) / errs.length;
    let stars = 1;
    if (avg < 30) stars = 5;
    else if (avg < 60) stars = 4;
    else if (avg < 110) stars = 3;
    else if (avg < 200) stars = 2;
    onFinish({
      stars,
      statLabel: "平均タイミング誤差",
      statValue: `${avg.toFixed(0)}ms`,
      rawValue: avg,
    });
  };

  const tap = () => {
    if (beatIndex < 0 || done) return;
    ...
    <div style={{ textAlign: "center", marginTop: 20 }}>
      <div
        style={{
          ...
          borderRadius: "50%",
          margin: "0 auto 20px",
          background: flash ? "#c9a227" : "#181a20",
          border: "2px solid #b8342a",
          transition: "background 0.08s",
        }}
      />
      <div style={{ fontSize: 12, color: "#8a8f9a", marginBottom: 16 }}>
        {beatIndex < 0 ? `テンポ${BPM}で4回鳴る。合わせてタップ` : done ? "審査終了" : `${beatIndex + 1}拍目……`}
      </div>
      {beatIndex < 0 ? (
        <button onClick={start} style={btnStyle("#b8342a")}>
          START
        </button>
      ) : (
        <button onClick={tap} disabled={done} style={{ ...btnStyle("#c9a227"), width: "60%" }}>
          TAP
        </button>
      )}
    </div>
...
// ============ ミニゲーム: 二重ゲージ審査 ============
function DualGaugeGame({ onFinish }) {
  const [targetA] = useState(() => 15 + Math.random() * 70);
  const [targetB] = useState(() => 15 + Math.random() * 70);
  const [pressing, setPressing] = useState(false);
  ...
  const press = () => {
    ...
    const tick = () => {
      ...
      setValA(triangleWave(t, 1100));
      setValB(triangleWave(t, 1700));
      rafRef.current = requestAnimationFrame(tick);
    };
    rafRef.current = requestAnimationFrame(tick);
  };
  ...
  const release = () => {
    ...
    setPressing(false);
    setDone(true);
    const errA = Math.abs(valA - targetA);
    const errB = Math.abs(valB - targetB);
    const total = errA + errB;
    let stars = 1;
    if (total < 4) stars = 5;
    else if (total < 10) stars = 4;
    else if (total < 20) stars = 3;
    else if (total < 35) stars = 2;
    onFinish({
      stars,
      statLabel: "2ゲージ合計ズレ",
      statValue: `${total.toFixed(1)}pt`,
      rawValue: total,
    });
  };
  ...
  const gaugeBar = (val, target, color) => (
    <div style={{ position: "relative", height: 22, borderRadius: 4, background: "#20232a", overflow: "hidden", border: "1px solid #2a2d34", marginBottom: 10 }}>
      <div style={{ position: "absolute", left: `${target - 7}%`, width: "14%", top: 0, bottom: 0, background: "rgba(201,162,39,0.35)" }} />
      <div style={{ position: "absolute", left: `${val}%`, top: 0, bottom: 0, width: 3, background: color, transform: "translateX(-50%)" }} />
    </div>
  );

  return (
    <div style={{ marginTop: 24 }}>
      <div style={{ fontSize: 11, color: "#6b6a63", marginBottom: 10, textAlign: "center" }}>
        2本同時に目標帯を狙う
      </div>
      {gaugeBar(valA, targetA, "#e8e6df")}
      {gaugeBar(valB, targetB, "#c9a227")}
      <div style={{ textAlign: "center" }}>
        <button
          onMouseDown={press}
          onTouchStart={press}
...
// ============ ミニゲーム: 四点同時審査 ============
function QuadTapGame({ onFinish }) {
  const areaRef = useRef(null);
  const timesRef = useRef([null, null, null, null]);
  ...
  const finalize = () => {
    if (doneRef.current) return;
    if (timesRef.current.some((t) => t === null)) return;
    doneRef.current = true;
    playTap();
    ...
    let stars = 1;
    if (diff < 25) stars = 5;
    else if (diff < 60) stars = 4;
    else if (diff < 120) stars = 3;
    else if (diff < 220) stars = 2;
    onFinish({
      stars,
      statLabel: "4点タップの最大時間差",
      statValue: `${diff.toFixed(0)}ms`,
      rawValue: diff,
    });
  };
  ...
  const handleTouchStart = (e) => {
    ...
    for (const t of e.changedTouches) {
      ...
      POS.forEach((p, i) => {
        if (timesRef.current[i] === null) {
          const cx = rect.width * p.x;
          const cy = rect.height * p.y;
          if (Math.hypot(x - cx, y - cy) < r) {
            timesRef.current[i] = performance.now();
            setDoneFlags((f) => {
              const nf = [...f];
              nf[i] = true;
              return nf;
  ...
  return (
    <div style={{ textAlign: "center", marginTop: 16 }}>
      <div style={{ fontSize: 12, color: "#8a8f9a", marginBottom: 10 }}>
        4本指で4点を同時にタップ(スマホでのみ判定可能)
      </div>
      <div
        ref={areaRef}
        onTouchStart={handleTouchStart}
        style={{ position: "relative", height: 260, border: "1px dashed #3a3d45", borderRadius: 4, touchAction: "none" }}
      >
        {POS.map((p, i) => (
          <div
            key={i}
            style={{
              ...
              height: 46,
              borderRadius: "50%",
              background: doneFlags[i] ? "#c9a227" : "rgba(184,52,42,0.3)",
              border: "2px solid #b8342a",
              transform: "translate(-50%,-50%)",
            }}
          />
...
// ============ ミニゲーム: 記憶審査(Simon式) ============
const SIMON_COLORS = ["#b8342a", "#c9a227", "#2e7d4f", "#3a5a9e"];
function SimonGame({ onFinish }) {
  ...
  const tapColor = (i) => {
    ...
    playTap();
    setLitIndex(i);
    setTimeout(() => setLitIndex(-1), 180);
    if (sequence[userStep] === i) {
      const next = userStep + 1;
      if (next === sequence.length) {
        if (sequence.length >= 8) {
          finalize(sequence.length);
          return;
        }
        const seq2 = [...sequence, Math.floor(Math.random() * 4)];
        setSequence(seq2);
        setTimeout(() => startRound(seq2), 500);
      } else {
        setUserStep(next);
      }
    } else {
      finalize(sequence.length - 1);
    }
  };

  const finalize = (correctRounds) => {
    setPhase("done");
    let stars = 1;
    if (correctRounds >= 8) stars = 5;
    else if (correctRounds >= 6) stars = 4;
    else if (correctRounds >= 4) stars = 3;
    else if (correctRounds >= 2) stars = 2;
    onFinish({
      stars,
      statLabel: "正しく記憶できた回数",
      statValue: `${correctRounds}回`,
      rawValue: correctRounds,
    });
  };

  return (
    <div style={{ textAlign: "center", marginTop: 20 }}>
      <div style={{ fontSize: 12, color: "#8a8f9a", marginBottom: 14 }}>
        {phase === "idle" && "光る順番を覚えて、同じ順にタップ"}
        {phase === "showing" && "よく見て……"}
        {phase === "input" && `${userStep + 1}番目をタップ`}
        {phase === "done" && "審査終了"}
      </div>
...
// ============ ミニゲーム: 三連審査(3秒→7秒→12秒) ============
function ChainTimingGame({ onFinish }) {
  const TARGETS = [3, 7, 12];
  const [phase, setPhase] = useState("idle"); // idle, running, done
  ...
  const mark = () => {
    ...
    const err = Math.abs(now - TARGETS[step]);
    ...
    if (step < 2) {
      ...
      cancelAnimationFrame(rafRef.current);
      setPhase("done");
      const total = nextErrors.reduce((a, b) => a + b, 0);
      let stars = 1;
      if (total < 0.3) stars = 5;
      else if (total < 0.8) stars = 4;
      else if (total < 1.8) stars = 3;
      else if (total < 3.5) stars = 2;
      onFinish({
        stars,
        statLabel: "3回分の合計誤差",
        statValue: `${total.toFixed(2)}秒`,
        rawValue: total,
      });
    }
  };
  ...
  return (
    <div style={{ textAlign: "center" }}>
      <div style={{ fontSize: 12, color: "#8a8f9a", margin: "16px 0 6px" }}>
        {phase === "idle" ? "3秒→7秒→12秒を順番に止める" : phase === "running" ? `目標:${TARGETS[step]}秒` : "審査終了"}
      </div>
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 48, margin: "10px 0 20px", color: "#e8e6df" }}>
        {phase === "idle" ? "--.---" : elapsed.toFixed(3)}
      </div>
      {phase !== "done" &&
        (phase === "idle" ? (
          ...
          <button onClick={mark} style={btnStyle("#c9a227")}>
            {step + 1}回目を止める
          </button>
        ))}
    </div>
...
function DialGame({ onFinish }) {
  ...
  const onUp = () => {
    ...
    let stars = 1;
    if (diff < 2) stars = 5;
    else if (diff < 6) stars = 4;
    else if (diff < 15) stars = 3;
    else if (diff < 30) stars = 2;
    onFinish({
      stars,
      statLabel: "角度誤差",
      statValue: `${diff.toFixed(1)}°`,
      rawValue: diff,
    });
  };
  ...
  return (
    <div style={{ textAlign: "center", marginTop: 16 }}>
      <div style={{ fontSize: 12, color: "#8a8f9a", marginBottom: 10 }}>目標角度:{target}°</div>
      <div
        ref={areaRef}
        onMouseDown={onDown}
        onMouseMove={onMove}
      ...
      >
        <div
          style={{
            ...
            width: 4,
            height: 80,
            background: done ? "#c9a227" : "#b8342a",
            transformOrigin: "top center",
            transform: `translateX(-50%) rotate(${angle}deg)`,
          }}
        />
      </div>
      <div style={{ fontSize: 11, color: "#6b6a63", marginTop: 12 }}>
        {done ? "審査終了" : "指で円周をなぞって回し、離して確定"}
      </div>
    </div>
  );
}
...
// ============ ミニゲーム: 角度審査(フリック方向) ============
function FlickAngleGame({ onFinish }) {
  const [target] = useState(() => Math.floor(Math.random() * 360));
  const [done, setDone] = useState(false);
  ...
  const onUp = (e) => {
    ...
    let stars = 1;
    if (diff < 3) stars = 5;
    else if (diff < 8) stars = 4;
    else if (diff < 18) stars = 3;
    else if (diff < 35) stars = 2;
    onFinish({
      stars,
      statLabel: "角度誤差",
      statValue: `${diff.toFixed(1)}°`,
  ...
  return (
    <div style={{ textAlign: "center", marginTop: 16 }}>
      <div style={{ fontSize: 12, color: "#8a8f9a", marginBottom: 10 }}>目標角度:{target}° の方向へフリック</div>
      <div
        onMouseDown={onDown}
        onMouseUp={onUp}
        onTouchStart={onDown}
      ...
      >
        <div style={{ fontSize: 12, color: "#6b6a63" }}>
          {done ? `あなたの角度:${resultAngle.toFixed(1)}°` : "枠内で好きな向きにフリック"}
        </div>
      </div>
    </div>
  );
...
function ReactionGame({ onFinish }) {
  const [phase, setPhase] = useState("idle"); // idle, waiting, active, done, fail
  ...
  const tap = () => {
    ...
    if (phase === "active") {
      ...
      let stars = 1;
      if (rt < 180) stars = 5;
      else if (rt < 260) stars = 4;
      else if (rt < 380) stars = 3;
      else if (rt < 550) stars = 2;
      onFinish({
        stars,
        statLabel: "反応時間",
        statValue: `${rt.toFixed(0)}ms`,
        rawValue: rt,
      });
    }
  };
    ...
    <div style={{ textAlign: "center", marginTop: 16 }}>
      <div
        ...
        style={{
          ...
          alignItems: "center",
          justifyContent: "center",
          background: phase === "active" ? "#2e7d4f" : "#181a20",
          border: "1px solid #2a2d34",
          transition: "background 0.05s",
        }}
      >
        <div style={{ fontSize: 13, color: phase === "active" ? "#0a1a10" : "#8a8f9a" }}>
          {phase === "idle" && "準備ができたらSTART"}
          {phase === "waiting" && "まだ……我慢……"}
          {phase === "active" && "今だ！タップ！"}
          {phase === "fail" && "フライング。審査終了。"}
          {phase === "done" && "審査終了"}
        </div>
      </div>
      {phase === "idle" && (
...
// ============ ミニゲーム: 暗記審査(1-back) ============
function NBackGame({ onFinish }) {
  const ROUNDS = 8;
  const [phase, setPhase] = useState("idle"); // idle, show, answer, done
  ...
  const nextRound = (r) => {
    const isMatch = r > 0 && Math.random() < 0.4;
    const num = isMatch ? prevRef.current : Math.floor(Math.random() * 9);
    isMatchRef.current = isMatch && num === prevRef.current;
    setCurrent(num);
    setPhase("answer");
  };
  ...
  const answer = (userSaysMatch) => {
    ...
    playTap();
    const correct = userSaysMatch === isMatchRef.current;
    ...
    if (nextRoundNum >= ROUNDS) {
      setPhase("done");
      let stars = 1;
      if (nextWrong === 0) stars = 5;
      else if (nextWrong === 1) stars = 4;
      else if (nextWrong <= 2) stars = 3;
      else if (nextWrong <= 4) stars = 2;
      onFinish({
        stars,
        statLabel: "誤答数",
        statValue: `${nextWrong}/${ROUNDS}回`,
        rawValue: nextWrong,
      });
    } else {
  ...
  return (
    <div style={{ textAlign: "center", marginTop: 20 }}>
      {phase === "idle" && (
        <>
          <div style={{ fontSize: 12, color: "#8a8f9a", marginBottom: 16 }}>
            数字が1つずつ出る。「1つ前と同じ数字か」を判定せよ(全{ROUNDS}回)
          </div>
          <button onClick={start} style={btnStyle("#b8342a")}>
            START
          </button>
      ...
      {phase === "answer" && (
        <>
          ...
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 64, margin: "16px 0 24px", color: "#e8e6df" }}>
            {current}
          </div>
          <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
            <button onClick={() => answer(true)} style={{ ...btnStyle("#20232a"), padding: "14px 28px" }}>
              同じ
            </button>
            <button onClick={() => answer(false)} style={{ ...btnStyle("#20232a"), padding: "14px 28px" }}>
              違う
            </button>
          </div>
        </>
      )}
      {phase === "done" && <div style={{ fontSize: 13, color: "#8a8f9a" }}>審査終了</div>}
    </div>
  );
}

// ============ ミニゲーム: 連打審査・弐(3段階ダブルタップ) ============
function TripleTapGame({ onFinish }) {
  ...
  const tap = () => {
    ...
    const diff = (performance.now() - firstTapRef.current) / 1000;
    const err = Math.abs(diff - TARGETS[stage]);
    const nextErrors = [...errors, err];
    setErrors(nextErrors);
    ...
    if (stage < 2) {
      ...
      const total = nextErrors.reduce((a, b) => a + b, 0);
      let stars = 1;
      if (total < 0.03) stars = 5;
      else if (total < 0.08) stars = 4;
      else if (total < 0.18) stars = 3;
      else if (total < 0.35) stars = 2;
      onFinish({
        stars,
        statLabel: "3段階合計誤差",
        statValue: `${total.toFixed(3)}秒`,
        rawValue: total,
      });
    }
  };
  ...
  return (
    <div style={{ textAlign: "center", marginTop: 30 }}>
      <div style={{ fontSize: 13, color: "#8a8f9a", marginBottom: 20 }}>
        {done
          ? "審査終了"
          : `第${stage + 1}段階(目標${TARGETS[stage]}秒):${waitingSecond ? "2回目をタップ" : "1回目をタップ"}`}
      </div>
      <button onClick={tap} disabled={done} style={{ ...btnStyle("#b8342a"), width: "60%" }}>
        {done ? "終了" : "タップ"}
      </button>
    </div>
  );
...
// ゲームID→コンポーネントのマップ(リアルタイム対戦ルームで動的に描画するため)
const GAME_COMPONENT_MAP = {
  timing: TimingGame,
  tap: TapCenterGame,
  coin: CoinGame,
  power: PowerPressGame,
  janken: JankenGame,
  onechar: OneCharGame,
  stillness: StillnessGame,
  opinion: OpinionGame,
  doubletap: DoubleTapGame,
  colormatch: ColorMatchGame,
  swipe: SwipeGame,
  dualtap: DualTapGame,
  rhythm4: RhythmGame,
  dualgauge: DualGaugeGame,
  quadtap: QuadTapGame,
  simon: SimonGame,
  chain3: ChainTimingGame,
  dial: DialGame,
  flickangle: FlickAngleGame,
  reaction: ReactionGame,
  nback: NBackGame,
  tripletap: TripleTapGame,
};
...
// ============ 総合ランキング(全審査横断・世界) ============
function GlobalRankScreen({ userId, onClose }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <div style={{ fontSize: 10, letterSpacing: 3, color: "#8a7a4d" }}>世界総合審査</div>
        <div style={{ fontFamily: "'Shippori Mincho', serif", fontSize: 20, color: "#e8e6df", marginTop: 6 }}>
          🌍 総合ランキング
        </div>
        <div style={{ fontSize: 11, color: "#6b6a63", marginTop: 6 }}>全審査の経験値を合算した世界順位</div>
      </div>
      ...
      {state === "ok" &&
        rows.map((r, i) => {
          const lv = computeLevel(r.totalXp || 0).level;
          ...
          return (
            <div
              ...
              style={{
                ...
                marginBottom: 8,
                borderRadius: 3,
                border: isMe ? "1px solid #c9a227" : "1px solid #2a2d34",
                background: isMe ? "rgba(201,162,39,0.08)" : "#181a20",
              }}
            >
              <span style={{ color: i < 3 ? "#c9a227" : "#8a8f9a", width: 32, fontSize: 13 }}>
                {i + 1}位
              </span>
              <span style={{ flex: 1, marginLeft: 8, fontSize: 13, color: "#e8e6df" }}>
                {r.nickname || "名も無き審査官"}
                {isMe && <span style={{ color: "#c9a227" }}> (あなた)</span>}
              </span>
              <span style={{ fontSize: 12, color: "#8a8f9a", marginRight: 10 }}>Lv.{lv}</span>
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: "#c9a227" }}>
                {r.totalXp || 0}XP
              </span>
            </div>
          );
...
// ============ 質問投稿(二者択一の民意調査への提案) ============
function SubmitQuestionScreen({ onClose }) {
  const [text, setText] = useState("");
  ...
  const submit = async () => {
    ...
    playTap();
    setSubmitting(true);
    const res = await submitQuestionIdea(text);
    setResult(res);
    setSubmitting(false);
    if (res.ok) setText("");
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <div style={{ fontSize: 10, letterSpacing: 3, color: "#8a7a4d" }}>投稿窓口</div>
        <div style={{ fontFamily: "'Shippori Mincho', serif", fontSize: 18, color: "#e8e6df", marginTop: 6 }}>
          「二者択一の民意調査」への質問を投稿
        </div>
        <div style={{ fontSize: 11, color: "#6b6a63", marginTop: 10, lineHeight: 1.6 }}>
          どうでもいい二択の質問を考えて送ってください。すぐには反映されず、審査官(運営)が確認したうえで、良さそうなものだけ審査に追加されます。
        </div>
      </div>
      ...
      <textarea
        ...
        maxLength={100}
        rows={3}
        placeholder="例:カレーにフルーツを入れるのはアリ"
        style={{
          width: "100%",
          padding: "12px",
          fontSize: 14,
      ...
      />
      <div style={{ fontSize: 11, color: "#6b6a63", textAlign: "right", marginTop: 4 }}>
        {text.length}/100
      </div>
      ...
      <button
        ...
        style={{ ...btnStyle("#b8342a"), width: "100%", marginTop: 14 }}
      >
        {submitting ? "送信中……" : "投稿する"}
      </button>
...
// ============ リアルタイム対戦ルーム ============
function BattleRoomScreen({ userId, nickname, onClose }) {
  const roomGameChoices = GAME_ORDER.filter((id) => !GAMES[id].hidden);
  const [mode, setMode] = useState("menu"); // menu, joinInput, in-room
  const [selectedGame, setSelectedGame] = useState(roomGameChoices[0]);
  ...
  const startListening = (code) => {
    ...
    unsubRef.current = listenRoom(code, (data) => setRoom(data));
  };

  const handleCreate = async () => {
    playTap();
    const code = await createRoom(selectedGame, userId, nickname);
    setRoomCode(code);
    setRole("host");
    setMode("in-room");
    startListening(code);
  ...
  const handleJoin = async () => {
    ...
    if (!code) return;
    const res = await joinRoom(code, userId, nickname);
    if (!res.ok) {
  ...
  const handleReady = async () => {
    playTap();
    await setRoomReady(roomCode, role);
  };

  // 両者準備完了になったら開始(どちらかのクライアントが検知して1回だけ更新)
  useEffect(() => {
    if (
      room &&
      room.status === "ready" &&
      ...
      startRoomPlaying(roomCode);
    }
    if (room && room.status === "playing" && room.hostResult && room.guestResult && !finishedRef.current) {
  ...
  const handleGameFinish = async (result) => {
    await submitRoomResult(roomCode, role, result);
  };
  ...
  if (mode === "menu") {
    return (
      <div style={wrapStyleLocal}>
        <button
          onClick={onClose}
          style={{ background: "none", border: "none", color: "#8a8f9a", fontSize: 12, marginBottom: 16, cursor: "pointer", padding: 0 }}
        >
          ← 台帳に戻る
        </button>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <div style={{ fontSize: 10, letterSpacing: 3, color: "#8a7a4d" }}>同時審査</div>
          <div style={{ fontFamily: "'Shippori Mincho', serif", fontSize: 20, color: "#e8e6df", marginTop: 6 }}>
            ⚔️ リアルタイム対戦
          </div>
        </div>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontSize: 12, color: "#8a8f9a", marginBottom: 10 }}>部屋を作る(審査項目を選ぶ)</div>
          <select
            value={selectedGame}
          ...
          >
            {roomGameChoices.map((id) => (
              <option key={id} value={id}>
                {GAMES[id].name}
              </option>
            ))}
          </select>
          <button onClick={handleCreate} style={{ ...btnStyle("#b8342a"), width: "100%" }}>
            部屋を作る
          </button>
        </div>

        <div style={{ textAlign: "center", color: "#4a4230", fontSize: 12, margin: "20px 0" }}>・ ・ ・</div>

        <div>
          <div style={{ fontSize: 12, color: "#8a8f9a", marginBottom: 10 }}>部屋番号で参加する</div>
          <input
            type="text"
            value={joinCode}
            onChange={(e) => setJoinCode(e.target.value.toUpperCase())}
            maxLength={5}
            placeholder="部屋番号(5文字)"
            style={{
              width: "100%",
              padding: "12px",
              background: "#181a20",
          ...
          />
          <button onClick={handleJoin} style={{ ...btnStyle("#4a2d6e"), width: "100%" }}>
            参加する
          </button>
          {error && <div style={{ fontSize: 12, color: "#a83232", marginTop: 8, textAlign: "center" }}>{error}</div>}
        </div>
      </div>
    );
  }
  ...
  if (!room) {
    return (
      <div style={wrapStyleLocal}>
        <div style={{ textAlign: "center", color: "#6b6a63", fontSize: 13, marginTop: 60 }}>接続中……</div>
      </div>
    );
  }

  const game = GAMES[room.gameId];
  const Comp = GAME_COMPONENT_MAP[room.gameId];
  const myReady = role === "host" ? room.hostReady : room.guestReady;
  const oppReady = role === "host" ? room.guestReady : room.hostReady;
  const myRes = role === "host" ? room.hostResult : room.guestResult;
  const oppRes = role === "host" ? room.guestResult : room.hostResult;
  const oppNickname = (role === "host" ? room.guestNickname : room.hostNickname) || "対戦相手";
  ...
  return (
    <div style={wrapStyleLocal}>
      ...
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <div style={{ fontSize: 10, letterSpacing: 3, color: "#8a7a4d" }}>部屋番号</div>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 26, color: "#c9a227", letterSpacing: 3 }}>
          {roomCode}
        </div>
        <div style={{ fontFamily: "'Shippori Mincho', serif", fontSize: 16, marginTop: 10, color: "#e8e6df" }}>
          {game.name}
        </div>
      </div>
      ...
      {room.status === "waiting" && (
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 13, color: "#8a8f9a", marginBottom: 16 }}>
            この番号を相手に伝えてください
          </div>
          <button onClick={copyCode} style={{ ...btnStyle("#c9a227"), width: "100%" }}>
            {copied ? "コピーしました" : "部屋番号をコピー"}
          </button>
          <div style={{ fontSize: 12, color: "#6b6a63", marginTop: 20 }}>相手の参加を待っています……</div>
        </div>
      )}

      {room.status === "ready" && (
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 13, color: "#8a8f9a", marginBottom: 6 }}>
            相手:{oppNickname}
          </div>
          <div style={{ fontSize: 12, color: "#6b6a63", marginBottom: 20 }}>
            {oppReady ? "相手は準備完了" : "相手はまだ準備中……"}
          </div>
          {!myReady ? (
            <button onClick={handleReady} style={{ ...btnStyle("#b8342a"), width: "100%" }}>
              準備完了
            </button>
          ) : (
            <div style={{ fontSize: 13, color: "#c9a227" }}>準備完了。相手を待っています……</div>
          )}
        </div>
      )}
      ...
      {room.status === "playing" && (
        <>
          {!myRes ? (
            Comp && <Comp onFinish={handleGameFinish} />
          ) : (
            <div style={{ textAlign: "center", fontSize: 13, color: "#8a8f9a", marginTop: 30 }}>
              結果を送信しました。相手の結果を待っています……
            </div>
          )}
        </>
      )}

      {room.status === "done" && myRes && oppRes && (
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 12, color: "#8a8f9a" }}>あなた:{myRes.statValue}</div>
          <div style={{ fontSize: 12, color: "#c9a227", marginTop: 2 }}>{"★".repeat(myRes.stars)}</div>
          <div style={{ margin: "16px auto", color: "#4a4230", fontSize: 12 }}>VS</div>
          <div style={{ fontSize: 12, color: "#8a8f9a" }}>{oppNickname}:{oppRes.statValue}</div>
          <div style={{ fontSize: 12, color: "#c9a227", marginTop: 2 }}>{"★".repeat(oppRes.stars)}</div>
          <div
            style={{
              marginTop: 24,
              fontFamily: "'Shippori Mincho', serif",
              fontSize: 22,
              color:
                myRes.rawValue === oppRes.rawValue
                  ? "#8a7a4d"
                  : (game.lowerIsBetter ? myRes.rawValue < oppRes.rawValue : myRes.rawValue > oppRes.rawValue)
                  ? "#2e7d4f"
                  : "#a83232",
            }}
          >
          ...
          <button onClick={leave} style={{ ...btnStyle("#20232a"), width: "100%", marginTop: 24 }}>
            台帳に戻る
          </button>
        </div>
      )}
...
// ============ プレイヤー設定(ニックネーム・引き継ぎ) ============
// ============ クレジット(エンドロール) ============
function CreditsStars({ count = 45 }) {
...
function CreditsScreen({ nickname, totalPlays, titlesCount, onClose }) {
  const INTRO_SEC = 2.2;
  const [phase, setPhase] = useState("intro"); // intro, rolling, ended
  const [duration, setDuration] = useState(34);
  const audioRef = useRef(null);
  const introTimerRef = useRef(null);

  const gameLines = GAME_ORDER.filter((id) => !GAMES[id].hidden).map((id) => GAMES[id].name);
  const hiddenLines = Object.values(GAMES)
    .filter((g) => g.hidden)
    .map((g) => g.name);
  ...
  const playFromStart = () => {
    ...
    introTimerRef.current = setTimeout(() => setPhase("rolling"), INTRO_SEC * 1000);
  };
  ...
  const handleLoadedMeta = () => {
    if (audioRef.current && audioRef.current.duration && isFinite(audioRef.current.duration)) {
      setDuration(Math.max(15, audioRef.current.duration - INTRO_SEC));
    }
  };

  return (
    <div style={{ position: "fixed", inset: 0, background: "#050608", zIndex: 80, overflow: "hidden" }}>
      <style>{`
      ...
      <audio ref={audioRef} src="/credits.mp3" onLoadedMetadata={handleLoadedMeta} />

      <div
        style={{
          position: "absolute",
          inset: 0,
          background: "radial-gradient(ellipse at 50% 30%, #14151c 0%, #050608 70%)",
          backgroundSize: "100% 200%",
          animation: phase === "rolling" ? `bgDrift ${duration}s linear forwards` : "none",
        }}
      />
      ...
      <div
        style={{
          ...
          lineHeight: 1.3,
          pointerEvents: "none",
          animation: "stampDrift 40s ease-in-out infinite alternate",
        }}
      >
        審査
        <br />
      ...
      {phase === "intro" && (
        <div
          style={{
            ...
            alignItems: "center",
            justifyContent: "center",
            animation: `introFadeScale ${INTRO_SEC}s ease-in-out forwards`,
          }}
        >
          <div style={{ fontSize: 11, color: "#8a7a4d", letterSpacing: 4 }}>WORLD DODEMOII BUREAU</div>
          <div
            style={{
              fontFamily: "'Shippori Mincho', serif",
              fontSize: 28,
      ...
      {phase !== "intro" && (
        ...
        >
          <div
            onAnimationEnd={() => setPhase("ended")}
            style={{
              position: "absolute",
              inset: 0,
              animation: phase === "rolling" ? `creditsScroll ${duration}s linear forwards` : "none",
              animationPlayState: phase === "rolling" ? "running" : "paused",
              textAlign: "center",
              color: "#e8e6df",
              fontFamily: "'Zen Kaku Gothic New', sans-serif",
              padding: "0 24px",
          ...
          >
            ...
            <div style={{ fontSize: 11, color: "#8a7a4d", letterSpacing: 4 }}>WORLD DODEMOII BUREAU</div>
            <div style={{ fontFamily: "'Shippori Mincho', serif", fontSize: 24, marginTop: 12 }}>
              世界一どうでもいいことを極める。
            </div>

            <Divider />
            <SectionLabel>企画・開発</SectionLabel>
            <div style={{ fontSize: 17, marginTop: 8, color: "#c9a227" }}>
              {nickname || "名も無き審査官"}
            </div>

            <Divider />
            <SectionLabel>審査項目</SectionLabel>
            {gameLines.map((n) => (
              <CreditLine key={n}>★ {n}</CreditLine>
            ))}

            <Divider />
            <SectionLabel>裏審査</SectionLabel>
            {hiddenLines.map((n) => (
              <CreditLine key={n} color="#c9a8e8">
                ★ {n}
              </CreditLine>
            ))}

            <Divider />
            <SectionLabel>審査記録</SectionLabel>
            <CreditLine>通算受審 {totalPlays ?? 0} 回</CreditLine>
            <CreditLine>獲得称号 {titlesCount ?? 0} 個</CreditLine>

            <Divider />
            <SectionLabel>使用技術</SectionLabel>
            <CreditLine>React</CreditLine>
            <CreditLine>Vite</CreditLine>
            <CreditLine>Firebase Firestore ── 審査記録管理</CreditLine>
            <CreditLine>Web Audio API ── 効果音</CreditLine>
            <CreditLine>Shippori Mincho / JetBrains Mono</CreditLine>

            <Divider />
            <SectionLabel>審査台帳運営</SectionLabel>
            <CreditLine>Vercel</CreditLine>

            <Divider />
            <SectionLabel>制作協力</SectionLabel>
            <CreditLine>Claude(Anthropic)</CreditLine>

            <Divider />
            <SectionLabel>テーマ曲</SectionLabel>
            <CreditLine>BREAK THE LIMIT</CreditLine>

            <Divider />
            <SectionLabel>制作の地</SectionLabel>
            <CreditLine>とあるスマートフォンの中</CreditLine>

            <Divider />
            <SectionLabel>特記事項</SectionLabel>
            <CreditLine>本作に実用的な意味は一切ありません</CreditLine>
            <CreditLine>それでもここまで極めたあなたは本物です</CreditLine>

            <div style={{ marginTop: 100, fontSize: 11, color: "#8a7a4d", letterSpacing: 3 }}>
              そして何より
            </div>
            <div style={{ fontSize: 16, marginTop: 10 }}>ここまで極めてくれた、あなたへ。</div>
      ...
      {phase !== "intro" && (
        <div
          style={{
            ...
            animation:
              phase === "rolling"
                ? `fadeToBlackNearEnd ${duration}s linear forwards`
                : phase === "ended"
                ? "none"
                : "none",
            zIndex: 79,
      ...
      {phase === "ended" && (
        ...
        >
          <div style={{ fontFamily: "'Shippori Mincho', serif", fontSize: 20, color: "#e8e6df" }}>
            END
          </div>
          <button onClick={playFromStart} style={{ ...btnStyle("#b8342a"), marginTop: 6 }}>
            もう一度見る
          </button>
          <button
            onClick={onClose}
...
function SectionLabel({ children }) {
  return (
    <div style={{ marginTop: 16, fontSize: 11, color: "#8a7a4d", letterSpacing: 3 }}>{children}</div>
  );
}
function CreditLine({ children, color }) {
  return (
    <div style={{ fontSize: 14, marginTop: 9, color: color || "#e8e6df" }}>{children}</div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 28, textAlign: "center" }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 24, color: "#c9a227" }}>
          Lv.{level}
        </div>
        <div
          style={{
        ...
        <div style={{ fontSize: 11, color: "#6b6a63", marginTop: 4 }}>
          {xpIntoLevel} / {xpForNext} XP(累計{totalXp}XP)
        </div>
      </div>

      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, color: "#8a8f9a", marginBottom: 10 }}>審査台帳のテーマ</div>
        {THEMES.map((t) => {
          ...
          return (
            <div
              ...
              style={{
                ...
                marginBottom: 8,
                borderRadius: 3,
                border: selected ? "1px solid #c9a227" : "1px solid #2a2d34",
                background: unlocked ? "#181a20" : "#14151a",
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                />
                <span style={{ fontSize: 13, color: "#e8e6df" }}>{t.name}</span>
              </div>
              <span style={{ fontSize: 11, color: unlocked ? "#c9a227" : "#6b6a63" }}>
                {selected ? "使用中" : unlocked ? "選択" : `Lv.${t.unlockLevel}で解放`}
              </span>
            </div>
          );
        })}
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      const rawXp = localStorage.getItem("dodemoii-xp");
      if (rawXp) setTotalXp(Number(rawXp) || 0);
      const rawTheme = localStorage.getItem("dodemoii-theme");
      if (rawTheme) setThemeId(rawTheme);
    } catch (e) {
      // no record yet
    }
    setLoading(false);
  ...
  const currentLevel = computeLevel(totalXp).level;
  const changeTheme = (id) => {
    const t = getTheme(id);
    if (t.unlockLevel > currentLevel) return;
    playTap();
    setThemeId(id);
    try {
      localStorage.setItem("dodemoii-theme", id);
    } catch (e) {
      // 保存できなくても続行
    }
  };
  ...
  const closeCredits = () => {
    ...
    if (bgmTrack !== "off" && audioElRef.current) {
  ...
  const backupToCloud = (nextRecords, nextTotal, nextStreak, nextXp) => {
    if (!userId) return;
    syncProfileToCloud(userId, {
      ...
      totalXp: nextXp !== undefined ? nextXp : totalXp,
    });
  };
  ...
  const restoreFromCode = async (code) => {
    ...
    try {
      ...
      localStorage.setItem("dodemoii-xp", String(nextXp));
  ...
  const handleFinish = (gameId, result) => {
    ...
    const gainedXp = xpForResult(result, isNewBest);
    const prevLevel = computeLevel(totalXp).level;
    const nextXp = totalXp + gainedXp;
    const newLevel = computeLevel(nextXp).level;
    setTotalXp(nextXp);
    try {
      localStorage.setItem("dodemoii-xp", String(nextXp));
    } catch (e) {
    ...
    if (newLevel > prevLevel) {
      const justUnlocked = THEMES.find((t) => t.unlockLevel === newLevel);
      setLevelToast({
        level: newLevel,
        themeName: justUnlocked ? justUnlocked.name : null,
      });
      vibrate([20, 40, 20, 40, 20, 40, 100]);
      setTimeout(() => setLevelToast(null), 4200);
    }

    backupToCloud(nextRec, nextTotal, nextStreak, nextXp);
  ...
  // 前の審査で★3以上を取らないと次が見えない、という順番ロック
  const visibleGameCards = [];
  for (let i = 0; i < GAME_ORDER.length; i++) {
    const id = GAME_ORDER[i];
    const prevId = i > 0 ? GAME_ORDER[i - 1] : null;
    const prevRec = prevId ? records[prevId] : null;
    const unlocked = i === 0 || (prevRec && prevRec.bestStars >= 3);
    if (unlocked) {
      visibleGameCards.push({ game: GAMES[id], locked: false });
    } else {
      visibleGameCards.push({ game: GAMES[id], locked: true });
      break; // これ以降はまだ見せない
    }
  }
  ...
  const themedWrap = { ...wrapStyle, background: theme.bg, color: theme.text };

  return (
    <div style={themedWrap}>
      <style>{`
      ...
      <header style={{ textAlign: "center", padding: "28px 18px 18px", borderBottom: `1px solid ${theme.border}` }}>
        <div style={{ fontSize: 10, letterSpacing: 4, color: theme.textDim }}>WORLD DODEMOII BUREAU</div>
        <h1
          style={{
            fontFamily: "'Shippori Mincho', serif",
            fontSize: 22,
        ...
        <div style={{ fontSize: 11, color: theme.textDim }}>
          {nickname ? `${nickname} ／ ` : ""}Lv.{currentLevel} ／ 通算受審 {totalPlays} 回
        </div>
        {streak >= 2 && (
      ...
      {levelToast && (
        <div
          style={{
            ...
            left: "50%",
            zIndex: 71,
            animation: "toastDrop 4.2s ease-in-out forwards",
            background: "#f3ecd9",
            color: "#241f16",
            padding: "12px 20px",
            borderRadius: 4,
        ...
        >
          <div style={{ fontSize: 10, color: "#8a7a4d", letterSpacing: 2 }}>レベルアップ</div>
          <div style={{ fontFamily: "'Shippori Mincho', serif", fontSize: 16, marginTop: 2 }}>
            Lv.{levelToast.level} に到達
          </div>
          {levelToast.themeName && (
            <div style={{ fontSize: 11, color: "#8a6f1a", marginTop: 4 }}>
              新テーマ「{levelToast.themeName}」解放！
            </div>
          )}
        </div>
      )}
      ...
      {screen === "hub" && (
        <div key="hub" className="screen-in" style={{ padding: "18px 16px 40px" }}>
          ...
          {visibleGameCards.map(({ game: g, locked }, i) => {
            const rec = records[g.id];
            if (locked) {
              return (
                <div
            ...
            return (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                  <span style={{ fontSize: 10, color: theme.textDim, letterSpacing: 1 }}>{g.no}</span>
                ...
                <div style={{ fontSize: 12, color: theme.textDim, marginTop: 4, lineHeight: 1.5 }}>
                  {g.desc}
                </div>
              </div>
            );
        ...
        <div key={screen} className="screen-in" style={{ padding: "18px 16px 40px" }}>
          ...
          {showLeaderboard ? (
            ...
            <>
              ...
              {screen === "coin_hidden" && (
                <CoinTossHiddenGame onFinish={(r) => handleFinish("coin_hidden", r)} />
              )}
              {screen === "rhythm4" && <RhythmGame onFinish={(r) => handleFinish("rhythm4", r)} />}
              {screen === "dualgauge" && (
                <DualGaugeGame onFinish={(r) => handleFinish("dualgauge", r)} />
              )}
              {screen === "quadtap" && <QuadTapGame onFinish={(r) => handleFinish("quadtap", r)} />}
              {screen === "simon" && <SimonGame onFinish={(r) => handleFinish("simon", r)} />}
              {screen === "chain3" && (
                <ChainTimingGame onFinish={(r) => handleFinish("chain3", r)} />
              )}
              {screen === "dial" && <DialGame onFinish={(r) => handleFinish("dial", r)} />}
              {screen === "flickangle" && (
                <FlickAngleGame onFinish={(r) => handleFinish("flickangle", r)} />
              )}
              {screen === "reaction" && (
                <ReactionGame onFinish={(r) => handleFinish("reaction", r)} />
              )}
              {screen === "nback" && <NBackGame onFinish={(r) => handleFinish("nback", r)} />}
              {screen === "tripletap" && (
                <TripleTapGame onFinish={(r) => handleFinish("tripletap", r)} />
              )}
            </>
          )}
        </div>
      ...
      {showGlobalRank && (
        <GlobalRankScreen userId={userId} onClose={() => setShowGlobalRank(false)} />
      )}

      {showSubmitQuestion && (
        <SubmitQuestionScreen onClose={() => setShowSubmitQuestion(false)} />
      )}
      ...
      {showBattleRoom && (
        <BattleRoomScreen
          userId={userId}
          nickname={nickname}
          onClose={() => setShowBattleRoom(false)}
        />
      )}
