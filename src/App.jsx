...
const GAMES = {
  ...
  onechar: {
    ...
    name: "文字を1文字だけ入力する",
    kanji: "一文字審査",
    desc: "入力欄に何か書いて確定せよ。ただし、ちょうど1文字だけが最高評価。",
...
function OneCharGame({ onFinish }) {
  ...
  const submit = () => {
    if (done) return;
    playTap();
    const len = [...text].length; // 絵文字等も1文字として数える
  ...
  return (
    <div style={{ textAlign: "center", marginTop: 24 }}>
      ...
      />
      <div style={{ fontSize: 11, color: "#6b6a63", marginTop: 8 }}>
        何文字書いても自由。ただし評価されるのは「ちょうど1文字」だけ。
      </div>
      <button onClick={submit} disabled={done} style={btnStyle("#b8342a")}>
        {done ? "審査終了" : "確定する"}
      </button>
    </div>
  );
